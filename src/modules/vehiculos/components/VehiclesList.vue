        ...
        <div class="overflow-hidden shadow">
          <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
            <thead class="bg-gray-100 dark:bg-gray-700">
              <tr>
                <th v-for="heading in ['Placa', 'Marca', 'Modelo', 'Año', 'Dueño', 'Estado', 'Actions']" :key="heading" scope="col" class="p-4 text-xs font-medium text-left text-gray-500 uppercase dark:text-gray-400">{{ heading }}</th>
              </tr>
            </thead>
            <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
              <tr v-if="isLoading"><td colspan="7" class="p-4 text-center text-gray-500 dark:text-gray-400">Cargando vehículos...</td></tr>
              <tr v-else-if="!vehicles.length"><td colspan="7" class="p-4 text-center text-gray-500 dark:text-gray-400">No se encontraron vehículos.</td></tr>
              <template v-else>
                <tr v-for="vehicle in vehicles" :key="vehicle.id" class="hover:bg-gray-100 dark:hover:bg-gray-700">
                  ...
                  <td class="p-4 text-gray-900 whitespace-nowrap dark:text-white"><span class="inline-flex items-center"><span class="w-2.5 h-2.5 mr-2 rounded-full" :class="vehicle.is_active ? 'bg-green-400' : 'bg-red-500'"></span>{{ vehicle.is_active ? 'Activo' : 'Inactivo' }}</span></td>
